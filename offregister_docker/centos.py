...
def install_docker0(*args, **kwargs):
    ...
    if tuple(imap(int, kernel_version[:kernel_version.find('.', 2)].split('.'))) < (3, 10):
    ...
    centos_join = partial(path.join,
                          path.join(path.dirname(
                              resource_filename(modules[__name__].__package__, '__init__.py')), '_centos'))
